...
public struct RiskModelParams<phantom T> has copy, drop {
...
public fun max_collateral_amount<T>(params: &RiskModelParams<T>): u64 {
    params.max_collateral_amount
}

public fun base_rate_per_sec<T>(params: &InterestModelParams<T>): u64 { params.base_rate_per_sec }

public fun interest_rate_scale<T>(params: &InterestModelParams<T>): u64 {
    params.interest_rate_scale
}

public fun borrow_rate_on_mid_kink<T>(params: &InterestModelParams<T>): u64 {
    params.borrow_rate_on_mid_kink
}

public fun mid_kink<T>(params: &InterestModelParams<T>): u64 { params.mid_kink }

public fun borrow_rate_on_high_kink<T>(params: &InterestModelParams<T>): u64 {
    params.borrow_rate_on_high_kink
}
...
public fun set_borrow_weight<T>(params: &mut InterestModelParams<T>, borrow_weight: u64) {
    params.borrow_weight = borrow_weight;
}
...
public fun gr_risk_model_params(): RiskModelParams<COIN_GR> {
    RiskModelParams {
        collateral_factor: 85,
        liquidation_factor: 95,
        liquidation_penalty: 7,
        liquidation_discount: 5,
        scale: 100,
        max_collateral_amount: math::pow(10, 9 + 7),
    }
...
public fun sui_risk_model_params(): RiskModelParams<SUI> {
    RiskModelParams {
        collateral_factor: 50,
        liquidation_factor: 60,
        liquidation_penalty: 7,
        liquidation_discount: 5,
        scale: 100,
        max_collateral_amount: math::pow(10, 9 + 7),
...
public fun usdc_interest_model_params(): InterestModelParams<USDC> {
    ...
    let borrow_rate_on_high_kink =
        50 * u64::mul_div(scale, interest_rate_scale, secs_per_year) / 100;
