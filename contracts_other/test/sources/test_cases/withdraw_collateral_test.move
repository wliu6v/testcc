...
public fun withdraw_collateral_without_borrowing_test() {
    ...
    test_scenario::next_tx(scenario, borrower);
    let withdrawed_collateral = withdraw_collateral::withdraw_collateral<ETH>(
        &version,
        &mut obligation,
        &obligation_key,
        &mut market,
...
public fun withdraw_collateral_with_borrowing_test() {
    ...
    // lender 先开 obligation
    let (mut lender_obligation, lender_obligation_key) = open_obligation_t(scenario, &version);
    deposit_collateral::deposit_collateral(
        &version,
        &mut lender_obligation,
        &mut market,
        usdc_coin,
        test_scenario::ctx(scenario),
    );
    ...
    test_scenario::return_shared(market);
    test_scenario::return_shared(obligation);
    test_scenario::return_shared(lender_obligation);
    test_scenario::return_to_address(admin, admin_cap);
    test_scenario::return_to_address(admin, x_oracle_policy_cap);
    test_scenario::return_to_address(borrower, obligation_key);
    test_scenario::return_to_address(lender, lender_obligation_key);
    test_scenario::end(scenario_value);
}
