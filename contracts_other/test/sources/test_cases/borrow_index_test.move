...
public fun borrow_index_test() {
    ...
    test_scenario::next_tx(scenario, borrower);
    let initial_borrow_index = market::borrow_index(&market, type_name::with_defining_ids<USDC>());
    ...
    let borrow_index = market::borrow_index(&market, type_name::with_defining_ids<USDC>());
    ...
    assert!(fixed_point32_empower::gte(index_precision, index_diff_rate), 0);
