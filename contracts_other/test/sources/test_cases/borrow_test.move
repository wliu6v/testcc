...
use protocol_test::constants::{
    usdc_interest_model_params,
    usdc_risk_model_params,
    eth_interest_model_params,
    eth_risk_model_params
};
...
public fun borrow_and_repay_unequal_debt_test() {
    ...
    let (reserve_debt_amount, _, _) = reserve::balance_sheet(balance_sheet);
    let market_borrow_index = market::borrow_index(&market, type_name::with_defining_ids<USDC>());

    let (obligation_debt_amount, obligation_debt_borrow_index) = obligation::debt(
        &obligation,
    ...
    let gusd_coin = coin::mint_for_testing<COIN_GUSD>(repay_amount, test_scenario::ctx(scenario));
    repay::repay<COIN_GUSD>(
        &version,
        &mut obligation,
        &mut market,
        gusd_coin,
